...
describe("VolunteerOrg - Applications API Tests", () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.userVolunteerOrg.deleteMany({ where: { email: { in: [organizerEmail, volunteerEmailApproved, volunteerEmailRejected] } } });
  ...
  // ✅ Apply (C)
  // =========================
  it("should apply to a volunteer activity", async () => {
    ...
    expect(appApproved.activityId).toBe(activityId);
    expect(appRejected.activityId).toBe(activityId);
  });
  ...
  it("should fetch all applications for an activity", async () => {
    ...
    const res = await getActivityApplications(req, { params: { id: activityId } });

    expect(res.status).toBe(200);
    const list = await res.json();
    expect(Array.isArray(list)).toBe(true);
    expect(list.length).toBeGreaterThan(0);
    expect(list[0].activityId).toBe(activityId);
  });
